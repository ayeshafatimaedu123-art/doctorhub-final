...
const seed = async () => {
  ...
  const { data: doctors, error: docError } = await supabase.from('doctors').insert([
    { user_id: doc1User.id, specialization: 'Cardiologist', treatment_type: 'Allopathic', diseases: ['Heart Disease','Hypertension','Chest Pain','Arrhythmia'], experience: 12, fee: 2000, qualification: 'MBBS, FCPS (Cardiology)', about: 'Experienced cardiologist.', city: 'Lahore', hospital: 'Services Hospital', rating: 4.8, total_reviews: 247, is_approved: true },
  ...
  if (docError) {
    console.error('❌ Doctors error:', docError.message);
    console.log('💡 Supabase SQL Editor mein yeh run karo:');
    console.log('   ALTER TABLE doctors DISABLE ROW LEVEL SECURITY;');
    process.exit(1);
  }
  ...
  const { data: clinics, error: clinicError } = await supabase.from('clinics').insert([
  ...
  if (clinicError) { console.error('❌ Clinics error:', clinicError.message); process.exit(1); }
